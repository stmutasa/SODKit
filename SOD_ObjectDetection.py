...
class MRCNN(SODMatrix):
    ...
    def _FRCNN_conv(self):
        ...
        with tf.variable_scope('fast_rcnn_net'):
            ...
            conv = self.linear_layer('Fc_2', conv, self.FCN, phase_train=self.phase_train)

            # Run them through the next FCN for class scores
            self.FCNN_box_logits = self.linear_layer('FCNN_classifier', conv, self.Num_classes + 1, phase_train=self.phase_train, relu=False, add_bias=False)

            # Run through the second FCN branch for box scores
            self.FCNN_class_logits = self.linear_layer('FCNN_regressor', conv, self.Num_classes * 4, phase_train=self.phase_train, relu=False, add_bias=False)
    ...
    def _ROI_Align(self):
        ...
        with tf.variable_scope('_ROI_Align'):
            ...
            for i in range (len(self.conv)):
                ...
                # TODO: Normalize the boxes to the Feature MAP
                boxes /= dims

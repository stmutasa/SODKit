...
class MRCNN(SODMatrix):
    ...
    def ROI_Align(self, rpn_proposal_boxes, rpn_proposal_scores, rpn_proposal_sources):
        ...
        with tf.variable_scope('ROI_Align'):
            ...
            for i in range (len(self.conv)):
                ...
                # Get valid indices
                vix = tf.where(tf.equal(self.conv[i].get_shape().as_list()[2], rpn_proposal_sources[:, 1]))

                # Gather all the indices from this FPN
                boxes = tf.gather(rpn_proposal_boxes, vix)
                sources = tf.gather(rpn_proposal_sources, vix)

                # TODO: Normalize the boxes!!
                ...
                roi.append(tf.image.crop_and_resize(self.conv[i], boxes, sources[:, 0], crop_size))

            # Combine all the crops
            rois = tf.concat(roi, axis=0)

            self.t1, self.t2, self.t3 = rpn_proposal_boxes, rpn_proposal_scores, rpn_proposal_sources
